...
const searchExam = (examList, søkeord) => {
    ...
    yearList.forEach((year, i) => {
        ...
        Object.keys(del1).forEach(oppg => {
            if(del1[oppg].includes(søkeord)) matching.push({year, oppg, link: examList[year].link});
        })
        if(exam[1]){
            const del2 = exam[1];
            Object.keys(del2).forEach(oppg => {
                if(del2[oppg].includes(søkeord)) matching.push({year, oppg, link: examList[year].link});
            })
        }
    });
